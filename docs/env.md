...
| `APP_BASE_URL` | `http://localhost:8000` | **Yes (prod)** | Used in email links and OAuth redirects |
| `MAIL_USERNAME` | *(empty)* | No | SMTP username (leave empty in dev) |
...
| `POSTGRES_PASSWORD` | `changeme` | Prod | PostgreSQL password (docker-compose.prod.yml) |
...
| Email | Links printed to console | Real SMTP required |
