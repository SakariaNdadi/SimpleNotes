...
def test_delete_label_unlinks_notes(auth_client, db, db_label, db_user):
    ...
    note = Note(user_id=user.id, description="Note linked to label", label_id=db_label.id)
    db.add(note)
    db.flush()
...
def test_create_duplicate_label_returns_422(auth_client):
    ...
    client.post("/labels", data={"title": "UniqueLabel", "description": "", "color": ""})
    r = client.post("/labels", data={"title": "UniqueLabel", "description": "", "color": ""})
    assert r.status_code == 422


def test_update_label_to_duplicate_title_returns_422(auth_client, db, db_label, db_user):
    ...
    r = client.put(f"/labels/{other.id}", data={
...
def test_update_nonexistent_label_returns_404(auth_client):
    ...
    client, _ = auth_client
    r = client.put(f"/labels/{uuid.uuid4()}", data={
