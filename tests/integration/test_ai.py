...
def test_post_llm_config_saves_and_sets_active(auth_client, db):
    ...
    config = db.query(UserLLMConfig).filter(
        UserLLMConfig.user_id == user.id, UserLLMConfig.model_name == "gpt-4o"
    ).first()
...
def test_put_llm_config_updates_model_name(auth_client, db, db_llm_config):
    ...
    client, _ = auth_client
    r = client.put(f"/settings/llm/{db_llm_config.id}", data={
        "provider_name": "openai",
...
def test_post_llm_activate_deactivates_others(auth_client, db, db_llm_config):
    ...
    second = UserLLMConfig(
        user_id=user.id, provider_name="anthropic",
        model_name="claude-3-opus", is_active=True,
    )
    db.add(second)
    db.flush()
...
def test_summarize_note_calls_ai_when_save_disabled(auth_client, db, db_note, db_llm_config):
...
def test_delete_summary(auth_client, db, db_note):
    ...
    found = db.query(NoteSummary).filter(
        NoteSummary.note_id == db_note.id, NoteSummary.user_id == user.id
    ).first()
    assert found is None
